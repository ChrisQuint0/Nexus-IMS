...
if (empty($selectedColumns)) {
    $selectedColumns = ['borrower_name', 'serial_no', 'item_name', 'claimed_at'];
}
...
try {
    ...
    $pdf->SetTitle('Claimed Items Logs Report');
    $pdf->SetSubject('Inventory of Claimed Items Logs');
    $pdf->SetKeywords('Inventory, Claimed Items, Logs, Report');
    ...
    // Title
    $title = "Claimed Items Logs Report";
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            foreach ($selectedColumns as $columnId) {
                ...
                } else if ($columnId == 'claimed_at' && !empty($value)) {
                    $date = new DateTime($value);
                ...
                if (in_array($columnId, ['department', 'borrower_name', 'item_name', 'staff'])) {
        ...
        $pdf->Cell($pageWidth, 10, 'No claimed items found in the selected date range', 1, 1, 'C');
    }
    ...
    $pdf->Cell(0, 8, "Total Number of Claimed Items Logs: " . $result->num_rows, 0, 1);
    ...
    $pdf->Output('claimed_logs_report_' . date('Y-m-d') . '.pdf', 'I');
    exit;
} catch (Exception $e) {
    // Handle any errors
