...
if (empty($selectedColumns)) {
    $selectedColumns = ['borrower_name', 'section', 'serial_no', 'item_name', 'MR'];
}
...
try {
    ...
    $pdf->SetTitle('Claimed Items Report');
    $pdf->SetSubject('Inventory of Claimed Items');
    $pdf->SetKeywords('Inventory, Claimed Items, Report');
    ...
    // Title
    $title = "Claimed Items Inventory Report";
    ...
    if ($result->num_rows > 0) {
        ...
        $pdf->Cell($pageWidth, 10, 'No claimed items found', 1, 1, 'C');
    }
    ...
    $pdf->Cell(0, 8, "Total Number of Records: " . $result->num_rows, 0, 1);
    ...
    $pdf->Output('claimed_items_report_' . date('Y-m-d') . '.pdf', 'I');
    exit;
} catch (Exception $e) {
    // Handle any errors
