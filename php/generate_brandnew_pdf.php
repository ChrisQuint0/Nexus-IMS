...
// Base query with joins - Note the status_id = 2 for brand new items
$baseQuery = "FROM
...
try {
    ...
    $pdf->SetTitle('Brand New Items Report');
    $pdf->SetSubject('Inventory of Brand New Items');
    $pdf->SetKeywords('Inventory, Brand New Items, Report');
    ...
    // Title
    $title = "Brand New Items Inventory Report";
    ...
    if ($result->num_rows > 0) {
        ...
        $pdf->Cell($pageWidth, 10, 'No brand new items found', 1, 1, 'C');
    }
    ...
    $pdf->Cell(0, 8, "Total Number of Brand New Items: " . $result->num_rows, 0, 1);
    ...
    $pdf->Output('brandnew_items_report_' . date('Y-m-d') . '.pdf', 'I');
    exit;
} catch (Exception $e) {
    // Handle any errors
