...
if (empty($selectedColumns)) {
    $selectedColumns = ['borrower_name', 'serial_no', 'reason'];
}
...
try {
    ...
    $sql = "SELECT 
            ...
            FROM 
                repair_logs rpl
            JOIN 
                gadget_distribution gd ON rpl.dist_id = gd.dist_id
            LEFT JOIN 
    ...
    $pdf->SetTitle('Repair Logs Report');
    $pdf->SetSubject('Inventory of Repair Logs');
    $pdf->SetKeywords('Inventory, Repair, Logs, Report');
    ...
    foreach ($selectedColumns as $columnId) {
        switch ($columnId) {
            ...
            case 'repair_date':
                ...
                break;
            case 'reason':
                $colWidths[$columnId] = $pageWidth * 0.25; // Extra space for repair reasons
                break;
            case 'log_id':
            case 'dist_id':
    ...
    if ($result->num_rows > 0) {
        while ($row = $result->fetch_assoc()) {
            foreach ($selectedColumns as $columnId) {
                ...
                } else if ($columnId == 'repair_date' && !empty($value)) {
                    $date = new DateTime($value);
                    $value = $date->format('M d, Y');
                }
                ...
                if (in_array($columnId, ['department', 'borrower_name', 'item_name', 'staff', 'reason'])) {
        ...
        $pdf->Cell($pageWidth, 10, 'No repair logs found in the selected date range', 1, 1, 'C');
    }
    ...
    $pdf->Cell(0, 8, "Total Number of Repair Logs: " . $result->num_rows, 0, 1);
    ...
    $pdf->Output('repair_logs_report_' . date('Y-m-d') . '.pdf', 'I');
    exit;
} catch (Exception $e) {
    // Handle any errors
